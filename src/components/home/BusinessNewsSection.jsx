...
function MetaRow({ article, showSave = false, isBookmarked = false, onBookmark }) {
  return (
    <div className="article-metaRow relative">
      ...
      {/* Save button inside MetaRow (desktop text cards) */}
      {showSave && (
        <button
          type="button"
          aria-label="સેવ કરો"
  ...
  return (
    ...
    >
      <div className="flex flex-col">
        <div className="mb-2.5 relative w-full aspect-[18/11] overflow-hidden rounded-[7px]">
          {article.img ? (
            <img
              src={article.img}
              alt=""
...
export default function BusinessNewsSection() {
  ...
  // Text stack FIRST (3 items), then 2 Image cards
  const textCards = nonHeroItems.slice(0, 3);
  const imageCards = nonHeroItems.slice(3, 5);

  if (!imageCards.length && !textCards.length) return null;

  return (
    <section className="mt-[3px] lg:items-stretch shadow-[0_2px_3px_rgba(0,0,0,0.06),0_0_20px_rgba(0,0,0,0.06)] p-7 lg:pt-7 lg:px-7 lg:pb-2 bg-white dark:bg-[#121212] border border-gray-200/70 dark:border-white/10 rounded-3xl">

      {/* 
        Mobile (<768)     : 1 column (text stack → image cards)
      ...
      */}
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 xl:items-stretch">
        ...
        <div className="flex flex-col justify-between gap-2  
                        md:col-span-2 
                        lg:col-span-1 lg:border-b-0 lg:pb-0">
          {textCards.map((article) => (
            <TextCard key={article.id} article={article} />
          ))}
        </div>

        {/* Section 2 & 3: Image Cards (side-by-side on md, original on lg) */}
